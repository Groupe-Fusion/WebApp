...
export function LivraisonExpressForm() {
	...
	const onSubmit = async (data: FormData) => {
		...
		try {
			...
			// Appel à l'API
			const response = await fetch("http://57.128.212.12:8082/api/reservations", {
				method: "POST",
				headers: {
	...
	return (
		<div className="w-full max-w-[1200px] relative min-h-screen">
			<Toast.Provider swipeDirection="right">
				<div className="mt-8 bg-white p-6 shadow sm:rounded-lg sm:px-10">
					<form className="space-y-8" onSubmit={handleSubmit(onSubmit)}>
						...
						<FormFieldset title="Informations destinataire">
							<div className="grid">
								<label className="text-sm font-semibold mb-1" htmlFor="receiverName">
									Nom complet
								</label>
								<input
									id="receiverName"
									className={`rounded-md border ${errors.receiverName ? "border-red-500" : "border-gray-300"} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
							...
							<div className="grid">
								<label className="text-sm font-semibold mb-1" htmlFor="receiverPhone">
									Téléphone
								</label>
								<input
									id="receiverPhone"
									type="tel"
									className={`rounded-md border ${errors.receiverPhone ? "border-red-500" : "border-gray-300"} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
									{...register("receiverPhone", {
										required: "Veuillez entrer le téléphone du destinataire",
									})}
								/>
								{errors.receiverPhone && <p className="text-xs text-red-500 mt-1">{errors.receiverPhone.message}</p>}
							</div>

							<div className="grid md:col-span-2">
								<label className="text-sm font-semibold mb-1" htmlFor="receiverAddress">
									Adresse
								</label>
								<input
									id="receiverAddress"
									className={`rounded-md border ${errors.receiverAddress ? "border-red-500" : "border-gray-300"} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
									{...register("receiverAddress", {
										required: "Veuillez entrer l'adresse du destinataire",
									})}
								/>
								{errors.receiverAddress && <p className="text-xs text-red-500 mt-1">{errors.receiverAddress.message}</p>}
							</div>

							<div className="grid">
								<label className="text-sm font-semibold mb-1" htmlFor="receiverCity">
									Ville
								</label>
								<input
									id="receiverCity"
									className={`rounded-md border ${errors.receiverCity ? "border-red-500" : "border-gray-300"} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
									{...register("receiverCity", {
										required: "Veuillez entrer la ville du destinataire",
									})}
								/>
								{errors.receiverCity && <p className="text-xs text-red-500 mt-1">{errors.receiverCity.message}</p>}
							</div>

							<div className="grid">
								<label className="text-sm font-semibold mb-1" htmlFor="receiverPostalCode">
									Code postal
								</label>
								<input
									id="receiverPostalCode"
									className={`rounded-md border ${errors.receiverPostalCode ? "border-red-500" : "border-gray-300"} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
									{...register("receiverPostalCode", {
										required: "Veuillez entrer le code postal du destinataire",
									})}
								/>
								{errors.receiverPostalCode && <p className="text-xs text-red-500 mt-1">{errors.receiverPostalCode.message}</p>}
							</div>
						</FormFieldset>
						...
						<FormFieldset title="Détails du colis">
							{/* Type de colis */}
							<div className="grid">
								<label className="text-sm font-semibold mb-1" htmlFor="packageType">
									Type de colis
								</label>
								<Controller
									name="packageType"
									...
									render={({ field }) => (
										<Select.Root onValueChange={field.onChange} value={field.value}>
											...
											<Select.Portal>
												<Select.Content className="bg-white rounded-md shadow-lg border border-gray-200 overflow-hidden z-50" position="popper">
													<Select.Viewport className="p-1">
														...
														<Select.Item
															...
															className="text-sm text-gray-800 rounded flex items-center h-8 px-2 relative select-none data-[highlighted]:bg-blue-100 data-[highlighted]:outline-none"
														>
															<Select.ItemText>Extra large (10-20kg)</Select.ItemText>
															<Select.ItemIndicator className="absolute right-2">
																<CheckIcon />
															</Select.ItemIndicator>
														</Select.Item>
							...
							<div className="grid">
								...
								<input
									...
									step="0.1"
									min="0"
									className={`rounded-md border ${errors.weight ? "border-red-500" : "border-gray-300"} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500`}
									{...register("weight", {
							...
							{/* Dimensions */}
							<div className="grid">
								<label className="text-sm font-semibold mb-1" htmlFor="dimensions">
							...
							<div className="flex items-center mt-4">
								...
								/>
								<label htmlFor="fragile" className="ml-2 block text-sm font-semibold">
									Contenu fragile
								</label>
							</div>
						</FormFieldset>
						...
						<FormFieldset title="Instructions spéciales">
							<div className="grid md:col-span-2">
								<label className="text-sm font-semibold mb-1" htmlFor="specialInstructions">
				...
				<Toast.Root
					className={`${status === "success" ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"} border fixed bottom-4 right-4 p-4 rounded-lg shadow-lg`}
					open={open}
					onOpenChange={setOpen}
					duration={5000}
				>
					<Toast.Title className={`font-medium ${status === "success" ? "text-green-800" : "text-red-800"}`}>
						{status === "success" ? "Demande envoyée avec succès!" : "Erreur lors de l'envoi du formulaire"}
					</Toast.Title>
					<Toast.Description className={`text-sm mt-1 ${status === "success" ? "text-green-600" : "text-red-600"}`}>
						{status === "success" ? "Votre demande de livraison express a été enregistrée." : "Veuillez réessayer ultérieurement ou contacter le service client."}
					</Toast.Description>
					<Toast.Action className="absolute top-2 right-2" asChild altText="Fermer">
