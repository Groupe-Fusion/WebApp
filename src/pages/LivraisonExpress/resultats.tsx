...
import { StarIcon, MapPinIcon, TruckIcon, ClockIcon, Filter, Check } from "lucide-react";
...
export default function LivraisonExpressResultats() {
	...
	const handleSelectProvider = async (provider: DeliveryProvider) => {
		...
		if (!reservationId) {
			...
			setError("Impossible de mettre à jour la réservation: identifiant manquant");
			return;
		}
		...
		try {
			...
			const response = await fetch(`http://57.128.212.12:8082/api/reservations/${reservationId}`, {
				method: "PUT",
				headers: {
	...
	useEffect(() => {
		...
		result = result.filter((provider) => provider.price >= priceRange[0] && provider.price <= priceRange[1]);
	...
	}, [allProviders, sortType, priceRange, maxDelay, showFastDeliveryOnly, showHighRatedOnly]);
	...
	const resetFilters = () => {
		setSortType("price");
		setPriceRange([minPrice, Math.ceil(maxPrice)]);
		setMaxDelay(null);
	...
	return (
		<div>
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
				<div>
					<h1 className="text-2xl font-bold text-gray-900">Prestataires disponibles</h1>
					<p className="text-sm text-gray-600 mt-1">{filteredProviders.length} prestataires trouvés</p>
				</div>
				...
				<div className="flex flex-wrap items-center gap-3">
					...
					>
						<ToggleGroup.Item
							value="price"
							className={`px-4 py-2 text-sm font-medium border ${sortType === "price" ? "bg-blue-50 text-blue-700 border-blue-200 z-10" : "bg-white text-gray-700 border-gray-300"}`}
						>
							Prix
						</ToggleGroup.Item>
						<ToggleGroup.Item
							value="delay"
							className={`px-4 py-2 text-sm font-medium border ${sortType === "delay" ? "bg-blue-50 text-blue-700 border-blue-200 z-10" : "bg-white text-gray-700 border-gray-300"}`}
						>
							Délai
						</ToggleGroup.Item>
					</ToggleGroup.Root>
					...
					<Popover.Root>
						...
						<Popover.Portal>
							<Popover.Content className="w-[300px] bg-white rounded-md shadow-lg border border-gray-200 p-4 z-50" sideOffset={5}>
								<div className="space-y-4">
									<div>
										<h3 className="text-sm font-medium mb-2 text-black">Prix (€)</h3>
									...
									<div>
										<h3 className="text-sm font-medium mb-2 text-black">Délai de livraison</h3>
										<select
											className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
											value={maxDelay === null ? "" : maxDelay.toString()}
											onChange={(e) => setMaxDelay(e.target.value ? parseInt(e.target.value) : null)}
										>
									...
									<div className="space-y-2">
										<div className="flex items-center">
											<Checkbox.Root
												...
												onCheckedChange={(checked) => setShowFastDeliveryOnly(checked === true)}
											>
												<Checkbox.Indicator>
													<Check className="h-3 w-3 text-blue-600" />
												</Checkbox.Indicator>
											</Checkbox.Root>
											<label htmlFor="fastDelivery" className="ml-2 block text-sm text-gray-700">
										...
										<div className="flex items-center">
											<Checkbox.Root
												...
												onCheckedChange={(checked) => setShowHighRatedOnly(checked === true)}
											>
												<Checkbox.Indicator>
													<Check className="h-3 w-3 text-blue-600" />
												</Checkbox.Indicator>
									...
									<div className="pt-2 flex justify-between">
										<button className="text-sm text-blue-600 hover:underline" onClick={resetFilters}>
											Réinitialiser les filtres
										</button>
										<Popover.Close asChild>
											<button className="px-3 py-1 text-sm rounded-md bg-blue-600 text-white hover:bg-blue-700">Appliquer</button>
										</Popover.Close>
									</div>
								</div>
			...
			{(showFastDeliveryOnly || showHighRatedOnly || maxDelay !== null || priceRange[0] > 0 || priceRange[1] < maxPrice) && (
				<div className="bg-blue-50 border border-blue-100 rounded-md p-3 mb-6">
					<div className="flex items-center justify-between">
						<div className="text-sm text-blue-700">
							<span className="font-medium">Filtres actifs:</span>{" "}
							{priceRange[0] > 0 || priceRange[1] < maxPrice ? `Prix entre ${priceRange[0].toFixed(2)}€ et ${priceRange[1].toFixed(2)}€` : ""}
							{maxDelay !== null ? ` • Délai maximum: ${maxDelay} min` : ""}
							{showFastDeliveryOnly ? " • Livraison rapide" : ""}
							{showHighRatedOnly ? " • Bien notés" : ""}
						</div>
						<button className="text-xs text-blue-600 hover:underline" onClick={resetFilters}>
							Effacer tout
						</button>
					</div>
				</div>
			...
			{loading ? (
				...
				<div className="bg-white shadow rounded-lg p-6 text-center">
					<h2 className="text-xl font-bold text-gray-600 mb-4">Aucun prestataire trouvé</h2>
					<p className="text-gray-600 mb-6">Aucun prestataire ne correspond à vos critères. Essayez d'ajuster vos filtres.</p>
					<button
						className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#2554f8] hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
